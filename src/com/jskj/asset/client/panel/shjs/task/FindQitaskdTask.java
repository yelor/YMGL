 ...
 * @author tt
 */
public abstract class FindQitaskdTask extends BaseTask{

    public static final Logger logger = Logger.getLogger(ChaXunTask.class);
    private final String CX_URI = Constants.HTTP + Constants.APPID + "qitashoukuandan/findall";
    public static final int pageSize = 10;
    private int pageIndex = 1;
    
    public FindQitaskdTask(int pageIndex){
        super();
        this.pageIndex = pageIndex;
    }
    
    public FindQitaskdTask(){
        this(1);
    }
    ...
    public Object doBackgrounp() {
        try{
            ...
            CommFindEntity<QitashoukuanshenqingDetailEntity> sqs = restTemplate.exchange(CX_URI,
                    HttpMethod.GET,
                    null,
                    new ParameterizedTypeReference<CommFindEntity<QitashoukuanshenqingDetailEntity>>() {
                    }).getBody();
            return sqs;
    ...
    public void onSucceeded(Object object) {
        ...
        if (object instanceof CommFindEntity) {
            responseResult((CommFindEntity<QitashoukuanshenqingDetailEntity>) object);
        } else {
            clientView.setStatus("response data is not a valid object", AssetMessage.ERROR_MESSAGE);
        }
    }

    public abstract void responseResult(CommFindEntity<QitashoukuanshenqingDetailEntity> response);
}
