 ...
 * @author woderchen
 */
public abstract class QitashoukuandanTask extends BaseTask {

    private static final Logger logger = Logger.getLogger(QitashoukuandanTask.class);
    private final String URI = Constants.HTTP + Constants.APPID + "qitashoukuandan/add";
    private QitashoukuanDetailEntity bean;

    public QitashoukuandanTask(QitashoukuanDetailEntity bean) {
        this.bean = bean;
    }
    ...
    public void onSucceeded(Object object) {
        ...
        if (object instanceof ComResponse) {
            responseResult((ComResponse<QitashoukuanDetailEntity>) object);
        } else {
            clientView.setStatus("response data is not a valid object", AssetMessage.ERROR_MESSAGE);
        }
    }

    public abstract void responseResult(ComResponse<QitashoukuanDetailEntity> response);
}
