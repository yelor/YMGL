 ...
 * @author woderchen
 */
public abstract class ShoukuanShenpiTask extends BaseTask {

    private static final Logger logger = Logger.getLogger(ShoukuanShenpiTask.class);
    private final String URI = Constants.HTTP + Constants.APPID + "shoukuandan/update";
    private ShoukuanshenqingDetailEntity bean;

    public ShoukuanShenpiTask(ShoukuanshenqingDetailEntity bean) {
        this.bean = bean;
    }
    ...
    public void onSucceeded(Object object) {
        ...
        if (object instanceof ComResponse) {
            responseResult((ComResponse<ShoukuanshenqingDetailEntity>) object);
        } else {
            clientView.setStatus("response data is not a valid object", AssetMessage.ERROR_MESSAGE);
        }
    }

    public abstract void responseResult(ComResponse<ShoukuanshenqingDetailEntity> response);
}
