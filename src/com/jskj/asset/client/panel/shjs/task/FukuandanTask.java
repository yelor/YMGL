 ...
 * @author woderchen
 */
public abstract class FukuandanTask extends BaseTask {

    private static final Logger logger = Logger.getLogger(FukuandanTask.class);
    private final String URI = Constants.HTTP + Constants.APPID + "fukuandan/add";
    private FukuanDetailEntity bean;

    public FukuandanTask(FukuanDetailEntity bean) {
        this.bean = bean;
    }
    ...
    public void onSucceeded(Object object) {
        ...
        if (object instanceof ComResponse) {
            responseResult((ComResponse<FukuanDetailEntity>) object);
        } else {
            clientView.setStatus("response data is not a valid object", AssetMessage.ERROR_MESSAGE);
        }
    }

    public abstract void responseResult(ComResponse<FukuanDetailEntity> response);
}
