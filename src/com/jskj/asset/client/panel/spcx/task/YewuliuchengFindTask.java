 ...
 * @author woderchen
 */
public abstract class YewuliuchengFindTask extends BaseTask {
    ...
    public YewuliuchengFindTask(HashMap map) {
        super();
        this.map = map;
    }
...
//            CommFindEntity<T> response = restTemplate.getForObject(URI + serviceId + "?pagesize=" + pageSize + "&pageindex=" + pageIndex, CommFindEntity.class);
            CommFindEntity<Yewuliuchengdetail> response = restTemplate.exchange(URI + "/spfind/yewuliucheng?" + paramater,
                    HttpMethod.GET,
                    null,
                    new ParameterizedTypeReference<CommFindEntity<Yewuliuchengdetail>>() {
                    }).getBody();
            return response;
    ...
    public void onSucceeded(Object object) {
        ...
        if (object instanceof CommFindEntity) {
            responseResult((CommFindEntity<Yewuliuchengdetail>) object);
        } else {
            clientView.setStatus("response data is not a valid object", AssetMessage.ERROR_MESSAGE);
        }
    }

    public abstract void responseResult(CommFindEntity<Yewuliuchengdetail> response);
}
