 ...
 * @author woderchen
 */
public final class DanweiUnitPanel extends BasePanel {

    private final static Logger logger = Logger.getLogger(DanweiUnitPanel.class);

    private DanWeiInfoJDialog childDialog;
    private int pageIndex;
    public int pageSize;
    private int count;

    private List<Unit> currentPageData;

    private final BindTableHelper<Unit> bindTable;

    /**
     * Creates new form NoFoundPane
     */
    public DanweiUnitPanel() {
        super();
        initComponents();
        pageIndex = 1;
        pageSize = 20;
        count = 0;
        bindTable = new BindTableHelper<Unit>(jTableDanjuleixing, new ArrayList<Unit>());
    ...
    private class RefreshTask extends UnitTask {
        ...
        public void onSucceeded(Object object) {
            ...
            if (unittbs != null && unittbs.getUnits() != null && unittbs.getUnits().size() > 0) {
                ...
                //存下所有的数据
                currentPageData = unittbs.getUnits();
    ...
    private void initComponents() {
        ...
        javax.swing.ActionMap actionMap = org.jdesktop.application.Application.getInstance(com.jskj.asset.client.AssetClientApp.class).getContext().getActionMap(DanweiUnitPanel.class, this);
        jButtonAdd.setAction(actionMap.get("add")); // NOI18N
        org.jdesktop.application.ResourceMap resourceMap = org.jdesktop.application.Application.getInstance(com.jskj.asset.client.AssetClientApp.class).getContext().getResourceMap(DanweiUnitPanel.class);
        ...
        SwingUtilities.invokeLater(new Runnable() {
            ...
            public void run() {
                if (childDialog == null) {
                    ...
                    childDialog = new DanWeiInfoJDialog(DanweiUnitPanel.this);
                    childDialog.setLocationRelativeTo(mainFrame);
                }
                childDialog.setUpdatedData(new Unit());
    ...
    public void update() {
        ...
        SwingUtilities.invokeLater(new Runnable() {
            @Override
            public void run() {
                Unit selectedData = selectedDataFromTable();
    ...
    public Task delete() {
        ...
        int result = AssetMessage.CONFIRM("确定删除数据:" + selectedData.getUnitName() + "");
        if (result == JOptionPane.OK_OPTION) {
            return new CommUpdateTask<Unit>(selectedData, "unit/delete/" + selectedData.getUnitId()) {
                @Override
                public void responseResult(ComResponse<Unit> response) {
                    if (response.getResponseStatus() == ComResponse.STATUS_OK) {
                        reload().execute();
                    } else {
                        AssetMessage.ERROR(response.getErrorMessage(), DanweiUnitPanel.this);
                    }
                }
    ...
    public List<Unit> getTableData() {
        return currentPageData;
    }

    public Unit selectedDataFromTable() {
    ...
    @Action
    public Task print() {
        UnitTask printData = new UnitTask(0, count) {
            @Override
            public void onSucceeded(Object object) {
                if (object instanceof Exception) {
                    Exception e = (Exception) object;
                ...
                UnitFindEntity unittbs = (UnitFindEntity) object;
                if (unittbs != null && unittbs.getUnits() != null && unittbs.getUnits().size() > 0) {
                    bindTable.createPrinter("单位", unittbs.getUnits()).buildInBackgound().execute();
                }
            }
        };
        return printData;
